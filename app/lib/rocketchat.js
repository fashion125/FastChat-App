...
const RocketChat = {
	...
	createChannel({ name, users, type }) {
		return call(
			type ? 'createChannel' : 'createPrivateGroup',
			name,
			users,
			type
		);
	...
	async testServer(url) {
		if (
			/^(https?:\/\/)?(((\w|[0-9-_])+(\.(\w|[0-9-_])+)+)|localhost)(:\d+)?$/.test(url)
		) {
	...
	connect(url, login) {
		return new Promise((resolve) => {
			...
			this.ddp.on('background', () =>
				this.getRooms().catch(e => console.warn('background getRooms', e)));
			...
			this.ddp.on(
				...
				protectedFunction((ddpMessage) => {
					...
					return reduxStore.dispatch(someoneTyping({
						_rid,
	...
	*/
		...
		try {
			...
			try {
				database.write(() => {
					const msg = database
						.objects('messages')
						.filtered('_id = $0', placeholder._id);
					database.delete(msg);
				});
			} catch (e) {
				console.error(e);
	...
	parsePermissions: permissions =>
		permissions.reduce((ret, item) => {
			ret[item._id] = item.roles.reduce(
				(roleRet, role) => [...roleRet, role.value],
				[]
			);
			return ret;
		}, {}),
	...
	hasPermission(permissions, rid) {
		...
		return permissions.reduce((result, permission) => {
			result[permission] = returnAnArray(allPermissions[permission]).some(item => mergedRoles.indexOf(item) !== -1);
			return result;
		}, {});
	}
};
